...
| Voice Admin Edge Functions (목록·집계·CSV·실시간 카드) | `backend/functions/voice-{responses,aggregates,csv-export,realtime}/index.ts` |
...
| 2026-05-20 | v1.1 — V-034 실시간 응답률·에러 카드: `voice-realtime` Edge Function (5m·1h·24h windows, dealer/visitor breakdown, normalize_queue 에러 count, last_response_at, lookback_min 분당 평균). Admin /voice/aggregates 상단에 RealtimeStrip 10s 폴링 — 부스 운영 중 실시간 가시성. event 필터 연동. |
| 2026-05-22 | v1.2 — V_60 Studio v2 (hd-design 와이어프레임 정합화): `035_studio_v2.sql`(studio_drafts 5컬럼·surveys version_label+brief_group_id·survey_questions ai_generated+edited_at + deploy_survey 7-arg RPC + version 자동 bump) + R_10.06 `voice_studio_survey_edit` 추가 + `shared/studio_lint.ts` 5종(time_overrun·missing_required·language_gap·merge_candidate·option_imbalance) + 4 Edge Functions(studio-build-survey 다중 target Promise.all·edit 분기·lint 통합, studio-deploy deployments[]·207 부분 성공, studio-load-survey 신규, studio-surveys-list 신규) + Admin Studio 페이지 전면 재작성(3-pane·5단계 wizard·9 컴포넌트 WizardProgress/BriefPane/ReviewPane/LintBanner/QuestionRow/PreviewPane/PhoneMockup/DeployCard/ExistingSurveyPicker + lib/studio_lint 클라용 4종). Dealer·Visitor 다중 선택 + 기존 설문 수정·재생성 플로우. CTT Moscow 데모(5/26) 직전 출시. |
