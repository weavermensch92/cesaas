...
export type BuilderStep = 'brief' | 'building' | 'review' | 'preview' | 'publishing' | 'done';

const STEPS: { key: BuilderStep; label: string; index: number }[] = [
  ...
  { key: 'publishing', label: 'Publish', index: 5 },
];

interface Props { current: BuilderStep; }
...
function stateOf(stepKey: BuilderStep, current: BuilderStep, currentIdx: number): StepState {
...
function badgeStyle(state: StepState): CSSProperties {
  const palette = state === 'done'
    ...
    : state === 'current' ? { bg: 'var(--hd-trust)', fg: '#fff' }
...
function labelStyle(state: StepState): CSSProperties {
  return { fontSize: 13, fontWeight: state === 'current' ? 600 : 500,
    color: state === 'pending' ? 'var(--hd-steel-500)' : 'var(--hd-trust)' };
}
function connectorStyle(active: boolean): CSSProperties {
  return { width: 24, height: 2, background: active ? 'var(--hd-accent-700)' : 'var(--hd-steel-200)', marginLeft: 4 };
}
